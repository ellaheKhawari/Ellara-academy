  ...
  return (
    <div>
      <ScrollReveal>
        ...
        <p className="mt-1 text-sm text-current/60">Your upcoming live sessions and practice rooms.</p>
      </ScrollReveal>
        ...
        {upcomingSessions.map((s, i) => (
          <ScrollReveal key={s.id} delay={i * 0.05}>
            <div className="flex items-center gap-4 rounded-2xl border border-black/[0.06] bg-white/60 p-5 dark:border-white/[0.08] dark:bg-white/[0.03]">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-ink/10 dark:bg-chrome/15 text-ink dark:text-chrome">
                <CalendarDays size={20} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="font-medium">{s.title}</p>
                <p className="mt-0.5 flex items-center gap-1.5 text-xs text-current/50">
                  <Clock size={12} /> {s.time} · {s.instructor}
                </p>
              </div>
