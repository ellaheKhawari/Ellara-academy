...
export function DashboardHome() {
    ...
    const sessionsCompletedCount = 42;
    const nextSessionTime = locale === "fa" ? "۶ بعدازظهر" : "6PM";

    const stats = [
        { icon: Flame, label: t.dashboard.currentStreak, value: `${formatNumber(user?.streakDays ?? 0, locale)} ${t.dashboard.days}` },
        { icon: TrendingUp, label: t.dashboard.fluencyScore, value: `${formatNumber(fluencyScore, locale)}/${formatNumber(100, locale)}` },
        { icon: BookOpenCheck, label: t.dashboard.sessionsCompleted, value: formatNumber(sessionsCompletedCount, locale) },
        {
            icon: Calendar,
            label: t.dashboard.nextSession,
            value: t.dashboard.nextSessionValue.replace("{day}", t.dashboard.today).replace("{time}", nextSessionTime),
        },
    ];
        ...
        <div>
                            ...
                            {skillBreakdown.map((s) => (
                                <div key={s.key}>
                                    <div className="flex justify-between text-xs">
                                        <span className="text-current/60">{t.skillBreakdown[s.key]}</span>
                                        <span className="font-mono-num text-current/50">{formatNumber(s.value, locale)}%</span>
                                    </div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
                        ...
                        <ul className="mt-4 space-y-3">
                            {upcomingSessions.map((u) => (
                                <li key={u.id} className="flex items-start gap-3 text-sm">
                                    ...
                                    <div>
                                        <p className="font-medium">{u.title[locale]}</p>
                                        <p className="text-xs text-current/50">
                                            {u.time[locale]} · {typeof u.instructor === "string" ? u.instructor : u.instructor[locale]}
                                        </p>
                                    </div>
                                </li>
                            ))}
                ...
                <ScrollReveal delay={0.05}>
                    <div className="h-full rounded-2xl border border-black/[0.06] bg-white/60 p-5 dark:border-white/[0.08] dark:bg-white/[0.03]">
                        <p className="text-sm font-medium">{t.dashboard.hoursStudied}</p>
